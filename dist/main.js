...
var LocalGPT = class extends import_obsidian3.Plugin {
  ...
  getActiveEditorView() {
    ...
    if (activeView) {
      const editorWithCm = activeView.editor;
      if (editorWithCm.cm instanceof import_view2.EditorView) {
  ...
  showActionContextMenu(editor) {
    ...
    if (!editorView) {
      new import_obsidian3.Notice("Error showing context menu: Active editor view not found.");
      return;
    }
      ...
      currentGroup.actions.forEach((action) => {
        ...
        contextMenu.addItem((item) => {
          item.setTitle(action.name).setIcon(action.replace ? "replace" : "plus-circle").onClick(async (evt) => {
            var _a2, _b2;
            const spinnerOnClick = editorView == null ? void 0 : editorView.plugin(spinnerPlugin);
            if (spinnerOnClick) {
              try {
